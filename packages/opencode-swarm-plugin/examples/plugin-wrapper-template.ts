...
const hive_query = tool({
	description:
		"Query cells with filters (replaces hive list, hive ready, hive wip)",
	args: {
		status: tool.schema
...
const swarm_worktree_create = tool({
	...
	args: {
		...
		task_id: tool.schema
			.string()
			.describe("Task/cell ID (e.g., cell-abc123.1)"),
...
const swarm_worktree_cleanup = tool({
	...
	args: {
		...
		task_id: tool.schema
			.string()
			.optional()
			.describe("Task/cell ID to clean up"),
...
function buildDynamicStateFromSnapshot(snapshot: SwarmStateSnapshot): string {
	...
	if (pending.length > 0) {
		...
		parts.push(
			`4. Spawn next subtask: \`swarm_spawn_subtask(cell_id="${next.id}", ...)\``,
		);
	}
	if (blocked.length > 0) {
