...
export const setCellsWorkingDirectory = setHiveWorkingDirectory;
export const getCellsWorkingDirectory = getHiveWorkingDirectory;
...
import { appendEvent, createEvent } from "swarm-mail";
...
export const CellError = HiveError;
...
// Legacy alias for backward compatibility
export const CellValidationError = HiveValidationError;

// ============================================================================
// Directory Migration (.cells → .hive)
// ============================================================================

/**
 * Result of checking if .cells → .hive migration is needed
 */
export interface MigrationCheckResult {
	/** Whether migration is needed */
	needed: boolean;
	/** Path to .cells directory if it exists */
	cellsPath?: string;
}

/**
 * Result of migrating .cells → .hive
 */
...
/**
 * Check if .cells → .hive migration is needed
 *
 * Migration is needed when:
 * - .cells directory exists
 * - .hive directory does NOT exist
 *
 * @param projectPath - Absolute path to the project root
 * @returns MigrationCheckResult indicating if migration is needed
 */
export function checkCellsMigrationNeeded(
	projectPath: string,
): MigrationCheckResult {
	const cellsDir = normalizePath(join(projectPath, ".cells"));
	...
	// If .cells exists but .hive doesn't, migration is needed
	if (existsSync(cellsDir)) {
		return { needed: true, cellsPath: cellsDir };
	}
...
/**
 * Migrate .cells directory to .hive

 * 
 * This function renames .cells to .hive. It should only be called
 * after user confirmation via CLI prompt.
 * 
 * @param projectPath - Absolute path to the project root
 * @returns MigrationResult indicating success or skip reason
 */
export async function migrateCellsToHive(
	projectPath: string,
	...
	if (!DbFileOps.exists(cellsDir)) {
		return {
			migrated: false,
			reason: ".cells directory not found - nothing to migrate",
		};
	}

	// Perform the rename with graceful retry pattern for Windows
	await DbFileOps.rename(cellsDir, hiveDir);
...
/**
 * Merge historic cells from cells.base.jsonl into issues.jsonl
 *
 * This function reads cells.base.jsonl (historic data) and issues.jsonl (current data),
 * merges them by ID (issues.jsonl version wins for duplicates), and writes the result
 * back to issues.jsonl.
 *
 * Use case: After migrating from .cells to .hive, you may have a cells.base.jsonl file
 * containing old cells that should be merged into the current issues.jsonl.
 *
 * @param projectPath - Absolute path to the project root
 * @returns Object with merged and skipped counts
 */
export async function mergeHistoricCells(
	projectPath: string,
): Promise<{ merged: number; skipped: number }> {
	const { readFileSync, writeFileSync, existsSync } = await import("node:fs");
	const hiveDir = normalizePath(join(projectPath, ".hive"));
	const basePath = normalizePath(join(hiveDir, "cells.base.jsonl"));
	...
	const baseLines = baseContent
		...
		.split("\n")
		.filter((l) => l);
	const baseCells = baseLines.map((line) => JSON.parse(line));

	// Read issues file (or create empty if missing)
	let issuesCells: AdapterCell[] = [];
	if (DbFileOps.exists(issuesPath)) {
		...
		const issuesLines = issuesContent
			...
			.split("\n")
			.filter((l) => l);
		issuesCells = issuesLines.map((line) => JSON.parse(line) as AdapterCell);
	}

	// Build set of existing IDs in issues.jsonl
	const existingIds = new Set(issuesCells.map((b) => b.id));

	// Merge: add cells from base that aren't in issues
	let merged = 0;
	let skipped = 0;

	for (const baseCell of baseCells as AdapterCell[]) {
		if (existingIds.has(baseCell.id)) {
			skipped++;
		} else {
			issuesCells.push(baseCell);
			merged++;
		}
	}
	...
	const mergedContent =
		issuesCells.map((b) => JSON.stringify(b)).join("\n") + "\n";
 ...
 *
 * On first initialization, checks for .cells/issues.jsonl and imports
 * historical cells if the database is empty.
 */
...
// Legacy alias for backward compatibility
export const getCellsAdapter = getHiveAdapter;
...
export const hive_create_epic = tool({
	...
	async execute(args, ctx) {
		// Be permissive: normalize first, then strictly validate
		const normalized = normalizeEpicCreateArgs(
			args,
		) as NormalizedEpicCreateArgs;
    ...
    { "title": "Add magic links", "priority": 2 }
		...
		try {
			...
			try {
				...
				// Non-fatal - log and continue
				console.warn(
					"[hive_create_epic] Failed to emit epic_created event:",
					error,
				);
			}
	...
	async execute(args, ctx) {
		...
		try {
			...
			const cell = await withRetry(() =>
				adapter.closeCell(projectKey, cellId, args.reason),
			);
			await adapter.markDirty(projectKey, cellId);
		...
		try {
			if (args.id) {
				...
				return JSON.stringify(
					matchingCells.map((c) => formatCellForOutput(c)),
					null,
					2,
				);
			}
			if (args.ready) {
				...
				return JSON.stringify(
					ready ? [formatCellForOutput(ready)] : [],
					null,
					2,
				);
			}
			...
			return JSON.stringify(
				cells.map((c) => formatCellForOutput(c)),
				null,
				2,
			);
		} catch (error: unknown) {
			...
			if (hasRemote) {
				...
				if (hasUnstagedChanges) {
					...
					const stashResult = await runGitCommand([
						...
						"-u",
						"-m",
						"hive_sync: auto-stash before pull",
					]);
				...
				try {
					...
					if (didStash) {
						...
						if (popResult.exitCode !== 0) {
							...
							console.warn(
								`[hive_sync] Warning: stash pop failed. Your changes are in 'git stash list'. Error: ${popResult.stderr}`,
							);
						}
					}
				}
 ...
 */
export const cells_create = tool({
	...hive_create,
	async execute(args, ctx) {
		warnDeprecated("cells_create", "hive_create");
 ...
 */
export const cells_create_epic = tool({
	...hive_create_epic,
	async execute(args, ctx) {
		warnDeprecated("cells_create_epic", "hive_create_epic");
 ...
 */
export const cells_query = tool({
	...hive_query,
	async execute(args, ctx) {
		warnDeprecated("cells_query", "hive_query");
 ...
 */
export const cells_update = tool({
	...hive_update,
	async execute(args, ctx) {
		warnDeprecated("cells_update", "hive_update");
 ...
 */
export const cells_close = tool({
	...hive_close,
	async execute(args, ctx) {
		warnDeprecated("cells_close", "hive_close");
 ...
 */
export const cells_start = tool({
	...hive_start,
	async execute(args, ctx) {
		warnDeprecated("cells_start", "hive_start");
 ...
 */
export const cells_ready = tool({
	...hive_ready,
	async execute(args, ctx) {
		warnDeprecated("cells_ready", "hive_ready");
 ...
 */
export const cells_sync = tool({
	...hive_sync,
	async execute(args, ctx) {
		warnDeprecated("cells_sync", "hive_sync");
 ...
 */
export const cells_link_thread = tool({
	...hive_link_thread,
	async execute(args, ctx) {
		warnDeprecated("cells_link_thread", "hive_link_thread");
