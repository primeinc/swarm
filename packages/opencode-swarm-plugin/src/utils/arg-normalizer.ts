...
function pick<T extends string>(obj: Record<string, unknown>, keys: readonly T[]): Partial<Record<T, unknown>> {
...
function aliasCommonKeys(obj: Record<string, unknown>): Record<string, unknown> {
...
export function normalizeCreateArgs(raw: unknown): unknown {
  ...
  return pick(obj, ["title", "type", "priority", "description", "parent_id"] as const);
}

export function normalizeUpdateArgs(raw: unknown): unknown {
  const obj = aliasCommonKeys(toPlainObject(raw));
  if (obj.status !== undefined) obj.status = normalizeStatus(obj.status) ?? obj.status;
  if (obj.priority !== undefined) obj.priority = coerceNumber(obj.priority);
...
export function normalizeCellsArgs(raw: unknown): unknown {
  ...
  if (obj.ready !== undefined) obj.ready = coerceBoolean(obj.ready);
  if (obj.limit !== undefined) obj.limit = coerceNumber(obj.limit);
  return pick(obj, ["id", "status", "type", "parent_id", "ready", "limit"] as const);
}
...
export function normalizeEpicCreateArgs(raw: unknown): unknown {
  ...
  if (obj.subtasks !== undefined && !Array.isArray(obj.subtasks)) {
    ...
    if (typeof obj.subtasks === "string") obj.subtasks = obj.subtasks.split(/[,\n]/).map(s => ({ title: s.trim() })).filter(x => x.title);
    else if (obj.subtasks && typeof obj.subtasks === "object") obj.subtasks = [obj.subtasks];
  }
  ...
  if (obj.epic_title === undefined && typeof raw === "string") obj.epic_title = String(raw);
  if (obj.epic_description !== undefined) obj.epic_description = String(obj.epic_description);

  return pick(obj, ["epic_title", "epic_description", "epic_id", "subtasks", "strategy", "task", "project_key", "recovery_context"] as const);
}

export function formatZodError(toolName: string, err: z.ZodError, example?: string): string {
