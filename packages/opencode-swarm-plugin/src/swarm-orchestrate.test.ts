...
describe("extractTechStack", () => {
	...
	test("extracts multiple technologies", () => {
		const task =
			"Build a Zod schema for validating Next.js API routes with TypeScript";
		const techStack = extractTechStack(task);
...
describe("finalizeEvalRecord integration", () => {
	...
	test("does not call finalizeEvalRecord when epic_id or project_path missing", async () => {
		...
		const testcellId = "cell-test123.0";
...
describe("anti-pattern auto-deprecation integration", () => {
	...
	test("extracts patterns from epic description on swarm_complete", async () => {
		...
		const epicResult = await hive_create_epic.execute(
			{
				...
				epic_description:
					"Split by feature: user CRUD operations, profile page, and settings. Tests alongside implementation.",
				subtasks: [
					{
		...
		expect(parsed.pattern_observations.extracted_patterns).toContain(
			"Split by feature",
		);
		expect(parsed.pattern_observations.extracted_patterns).toContain(
	...
	test("increments pattern success count on each completion", async () => {
		...
		const epicResult = await hive_create_epic.execute(
			{
				...
				epic_description:
					"Split by component: service, controller, middleware. Tests alongside implementation.",
				subtasks: [
					{
						title: "Auth service",
						priority: 2,
		...
		expect(parsed1.pattern_observations.extracted_patterns).toContain(
			"Split by component",
		);
		...
		expect(parsed2.pattern_observations.extracted_patterns).toContain(
			"Split by component",
		);
