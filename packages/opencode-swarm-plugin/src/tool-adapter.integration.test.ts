...
import { semantic_memory_find, semantic_memory_store } from "./hivemind-tools";
...
describe("swarmmail tools adapter wiring", () => {
	...
	it("swarmmail_release_all clears reservations across agents", async () => {
		...
		const reserveAfter = await executeTool<{ conflicts?: unknown[] }>(
			swarmmail_reserve,
			{
				paths: ["src/release-all-1.ts", "src/release-all-2.ts"],
				exclusive: true,
			},
			testerCtx,
		);
...
describe("hive tools adapter wiring", () => {
	...
	it("hive_start works without explicit dbOverride", async () => {
		...
		try {
			...
			const result = await executeToolRaw(hive_start, { id: created.id }, ctx);
		...
		try {
			...
			const epic = await executeTool<EpicCreateResult>(
				hive_create_epic,
				{
					...
					subtasks: [{ title: "Broadcast subtask 1", priority: 2 }],
				},
				ctx,
			);
		...
		try {
			...
			const epic = await executeTool<EpicCreateResult>(
				hive_create_epic,
				{
					...
					subtasks: [{ title: "Checkpoint subtask", priority: 2 }],
				},
				ctx,
			);
	...
	it("runs full workflow without adapter errors", async () => {
		...
		try {
			...
			const cell = await executeTool<cell>(
				hive_create,
				{
					title: "Smoke test workflow cell",
			...
			const memory = await executeTool<{ id: string }>(
				...
				{
					information:
						"Smoke test verified full tool adapter wiring works end-to-end",
