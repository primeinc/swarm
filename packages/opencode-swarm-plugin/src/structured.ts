...
import {
	type CellTree,
	CellTreeSchema,
	type CriterionEvaluationSchema,
	type Evaluation,
	EvaluationSchema,
...
/**
 * Parse and validate a cell tree (epic with subtasks)
 *
 * Validates the structure before creating cells.
 */
export const structured_parse_cell_tree = tool({
	description:
		"Parse and validate cell tree response. Uses CellTreeSchema. Validates before creating epic with subtasks.",
	args: {
		response: tool.schema
			.string()
			.describe("Agent response containing cell tree"),
	},
	async execute(args, ctx) {
		try {
			const [extracted, method] = extractJsonFromText(args.response);
			...
			if (error instanceof z.ZodError) {
				...
				return JSON.stringify(
					{
						success: false,
						error: "cell tree does not match schema",
						validation_errors: bullets,
						feedback: bullets.map((e) => `- ${e}`).join("\n"),
						expected_shape: {
