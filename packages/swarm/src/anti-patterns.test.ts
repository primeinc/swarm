...
describe("DecompositionPatternSchema", () => {
	it("validates a complete valid pattern", () => {
		const pattern = {
			...
			example_cells: ["cell-123", "cell-456"],
			tags: ["file-splitting"],
			reason: "Test pattern",
		};
...
describe("invertToAntiPattern", () => {
	...
	it("preserves example_cells", () => {
		const result = invertToAntiPattern(basePattern, "Test");
		expect(result.inverted.example_cells).toEqual(["cell-123", "cell-456"]);
	});
...
describe("recordPatternObservation", () => {
	...
	it("adds cell to example_cells when provided", () => {
		const result = recordPatternObservation(basePattern, true, "cell-789");
		expect(result.pattern.example_cells).toContain("cell-789");
	});
	...
	it("limits example_cells to MAX_EXAMPLE_cellS (10)", () => {
		...
		const result = recordPatternObservation(pattern, true, "cell-new");
		expect(result.pattern.example_cells.length).toBe(10);
		expect(result.pattern.example_cells).toContain("cell-new");
		expect(result.pattern.example_cells).not.toContain("cell-0"); // Oldest removed
	});
	...
	it("keeps newest cells when trimming example_cells", () => {
		...
		const result = recordPatternObservation(pattern, true, "cell-new");
		expect(result.pattern.example_cells[0]).toBe("cell-2"); // First one removed
		expect(result.pattern.example_cells[9]).toBe("cell-new"); // New one added
	});
