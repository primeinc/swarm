...
describe("generateReviewPrompt", () => {
	...
	it("includes downstream context (what depends on this)", () => {
		const contextWithDownstream: ReviewPromptContext = {
			...baseContext,
			downstream_tasks: [
				{ id: "cell-test-123.2", title: "Create auth middleware" },
				{ id: "cell-test-123.3", title: "Add protected routes" },
			],
		};
...
describe("swarm_review", () => {
	...
	it("returns JSON with review_prompt field", async () => {
		...
		expect(parsed.context.epic_id).toBe("cell-test-123");
		expect(parsed.context.task_id).toBe("cell-test-123.1");
	});

	it("includes remaining attempts in context", async () => {
		clearReviewStatus("cell-test-123.1");
		const result = await swarm_review.execute(
			{
				project_key: "/tmp",
				epic_id: "cell-test-123",
...
describe("swarm_review_feedback", () => {
	beforeEach(() => {
		clearReviewStatus("cell-feedback-test");
		vi.clearAllMocks();
	});
	...
	it("fails task after 3 rejected reviews", async () => {
		...
		// Check final state
		const status = getReviewStatus("cell-feedback-test");
	...
	it("extracts epic ID from task ID for thread", async () => {
		// Task ID format: cell-epic.subtask
		const result = await swarm_review_feedback.execute(
			{
				project_key: "/tmp/test-project",
				task_id: "cell-epic-123.4",
				worker_id: "worker-test",
				status: "approved",
			},
			mockContext,
		...
		const parsed = JSON.parse(result);
		expect(parsed.success).toBe(true);
		// The sendSwarmMessage mock was called with threadId = "cell-epic-123"
	});
});
...
describe("swarm_complete with review gate", () => {
	...
	beforeEach(() => {
		clearReviewStatus("cell-gate-test");
	});

	it("isReviewApproved returns false for unreviewed task", () => {
		expect(isReviewApproved("cell-gate-test")).toBe(false);
	});

	it("isReviewApproved returns true after markReviewApproved", () => {
		markReviewApproved("cell-gate-test");
		expect(isReviewApproved("cell-gate-test")).toBe(true);
	});

	it("getReviewStatus provides complete status info", () => {
		const status = getReviewStatus("cell-gate-test");
	...
	it("approval clears attempt count", async () => {
		...
		let status = getReviewStatus("cell-gate-test");
		...
		status = getReviewStatus("cell-gate-test");
		expect(status.attempt_count).toBe(0);
		expect(status.approved).toBe(true);
	});
});
...
describe("swarm_review_feedback retry_context", () => {
	beforeEach(() => {
		clearReviewStatus("cell-retry-test");
		vi.clearAllMocks();
	});
	...
	it("returns retry_context when status is needs_changes", async () => {
		...
		expect(parsed.retry_context).toHaveProperty("task_id", "cell-retry-test");
