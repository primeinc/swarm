...
import { getReviewStatus, isReviewApproved } from "./swarm-review";
import { canUseWorktreeIsolation, getStartCommit } from "./swarm-worktree";
import {
	checkAllTools,
...
async function getSubtaskFilesOwned(
	...
	try {
		...
		const decompositionEvent = events.find(
			(e: any) => e.type === "decomposition_generated" && e.epic_id === epicId,
		);

		if (!decompositionEvent) {
			console.warn(
				`[swarm_complete] No decomposition event found for epic ${epicId}`,
			);
			return null;
		}
		...
		if (!subtaskMatch) {
			console.warn(
				`[swarm_complete] Could not parse subtask index from ${subtaskId}`,
			);
			return null;
		}
		...
		if (subtaskIndex >= subtasks.length) {
			console.warn(
				`[swarm_complete] Subtask index ${subtaskIndex} out of range (${subtasks.length} subtasks)`,
			);
			return null;
		}
	...
	async execute(args) {
		...
		if (isolationMode === "worktree" && args.project_path) {
			...
			isolationInfo = {
				...
				available: false,
				reason:
					"Worktree mode requires project_path. Falling back to reservation mode.",
			};
			warnings.push(
	...
	async execute(args) {
		...
		if (args.status === "blocked" || args.status === "in_progress") {
			try {
				...
				console.error(
					`[swarm] Failed to update cell status: ${error instanceof Error ? error.message : String(error)}`,
				);
			}
		}
...
export const swarm_complete = tool({
	...
	args: {
		...
		start_time: tool.schema
			.number()
			.describe(
				"Task start timestamp (Unix ms) for duration calculation - REQUIRED for accurate analytics",
			),
...
Continuing with completion, but this should be fixed for future subtasks.`;
			...
			let contractValidation: { valid: boolean; violations: string[] } | null =
				null;
...
This will be recorded as a negative learning signal.`;
							...
							console.log(
								`[swarm_complete] Contract validation passed: all ${args.files_touched.length} files within owned scope`,
							);
						}
					} else {
						console.warn(
							`[swarm_complete] Could not retrieve files_owned for contract validation - skipping`,
						);
					}
				}
			}
			...
			try {
				...
				if (checkResult.rows.length > 0) {
					...
					await db.query(
						...
						[
							JSON.stringify({ completed: true, summary: args.summary }),
							deferredUrl,
						],
					);
			...
			try {
				...
				try {
					const syncResult = await hive_sync.execute(
						{ auto_pull: false },
						_ctx,
					);
			...
			try {
				const event = createEvent("subtask_outcome", {
					...
					success: true,
					scope_violation: contractValidation
			...
			try {
				const { scoreOutcome, OutcomeSignalsSchema } = await import(
					"./learning"
				);
				...
				const { outcomeToFeedback, scoreImplicitFeedback } = await import(
					"./learning"
				);
				const scoredOutcome = scoreImplicitFeedback(signals);
				const feedbackEvent = outcomeToFeedback(
					scoredOutcome,
					"task_completion",
				);
				...
				if (signals.strategy) {
					...
					if (maturity) {
						const allFeedback = await storage.getMaturityFeedback(
							signals.strategy,
						);
					...
					if (patterns.length > 0) {
						...
						for (const patternContent of patterns) {
							...
							const existingPatterns =
								await patternStorage.findByContent(patternContent);
							let pattern = existingPatterns[0];

							if (!pattern) {
								// Create new pattern
								pattern = createPattern(patternContent, [
									"decomposition",
									"auto-detected",
								]);
...
Files touched: ${args.files_touched?.join(", ") || "none recorded"}`,
			...
			try {
				...
				console.warn(
					"[swarm_complete] Failed to capture subtask outcome:",
					error,
				);
			}
			...
			try {
				...
				// Non-fatal - don't block completion if capture fails
				console.warn(
					"[swarm_complete] Failed to capture subtask_success:",
					error,
				);
			}
			...
			try {
				...
				console.warn(
					"[swarm_complete] Failed to capture subtask_failed:",
					captureError,
				);
			}
			...
			return JSON.stringify(
				{
					...
					recovery: {
						...
						common_fixes: {
							"Verification Gate":
								"Use skip_verification=true to bypass (not recommended)",
							"Cell close":
								"Check cell status with hive_query(), may need hive_update() first",
							"Self-evaluation":
								"Check evaluation JSON format matches EvaluationSchema",
						},
					},
				},
				null,
...
export const swarm_record_outcome = tool({
	...
	args: {
		...
		project_path: tool.schema
			...
			.optional()
			.describe(
				"Project path (for finalizing eval records when all subtasks complete)",
			),
		epic_id: tool.schema
			.string()
			.optional()
			.describe(
				"Epic ID (for finalizing eval records when all subtasks complete)",
			),
	},
	async execute(args) {
		// Build outcome signals
		...
		if (args.project_path && args.epic_id) {
			try {
				...
				// Non-fatal - log and continue
				console.warn(
					"[swarm_record_outcome] Failed to finalize eval record:",
					error,
				);
			}
		}
...
export const swarm_research_phase = tool({
	...
	args: {
		...
		check_upgrades: tool.schema
			.boolean()
			.optional()
			.describe("Compare installed vs latest versions (default: false)"),
	},
	async execute(args) {
