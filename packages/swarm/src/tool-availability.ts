 ...
 * - cells: DEPRECATED - Use hive instead (kept for backward compatibility)
...
const toolCheckers: Record<ToolName, () => Promise<ToolStatus>> = {
	...
	// The cells CLI is deprecated - always return false, use HiveAdapter instead
	cells: async () => {
		return {
			available: false,
			checkedAt: new Date().toISOString(),
			error:
				"The cells CLI is deprecated - use hive_* tools with HiveAdapter instead",
		};
	},
