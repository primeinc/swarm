...
async function main() {
  ...
  console.log(`   ✅ ${mixedSuccesses}/100 operations in ${Date.now() - mixedStart}ms`);
  ...
  console.log(`   ✅ ${txSuccesses}/10 transactions in ${Date.now() - txStart}ms`);
  ...
  // Final count
  const finalCount = await db.query<{ cnt: number }>("SELECT COUNT(*) as cnt FROM stress_test");
