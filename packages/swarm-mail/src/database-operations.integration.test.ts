...
describe("LibSQL Concurrency Tests", () => {
  ...
  test("100 mixed reads and writes succeed", async () => {
    ...
    const count = await db.query<{ cnt: number }>("SELECT COUNT(*) as cnt FROM test_data");
  ...
  test("transactions complete without SQLITE_BUSY", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      await db.transaction(async (tx) => {
        await tx.query("INSERT INTO tx_test (id, val) VALUES (?, ?)", [randomUUID(), i]);
        await tx.query("INSERT INTO tx_test (id, val) VALUES (?, ?)", [randomUUID(), i * 10]);
      });
    }

    const count = await db.query<{ cnt: number }>("SELECT COUNT(*) as cnt FROM tx_test");
...
describe("Vector Operations Tests", () => {
  ...
  test("vector_distance_cos returns correct similarity ranking", async () => {
    ...
    for (const doc of docs) {
      await client.execute({
        ...
        args: [randomUUID(), doc.content, JSON.stringify(Array.from(createNormalizedVector(doc.vector)))],
      });
    }
  ...
  test("concurrent vector reads succeed", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      await client.execute({
        ...
        args: [`concurrent-${i}`, `Content ${i}`, JSON.stringify(Array.from(createNormalizedVector([i / 5, 0, 0])))],
      });
    }
    ...
    const results = await Promise.allSettled(
      Array.from({ length: 20 }, (_, i) => {
        const queryVec = createNormalizedVector([Math.random(), Math.random(), 0]);
  ...
  test("cosine similarity values are normalized (0-2 range)", async () => {
    ...
    await client.execute({
      sql: `INSERT INTO memories (id, content, embedding) VALUES (?, ?, vector(?))`,
      args: ["same", "Same direction", JSON.stringify(Array.from(createNormalizedVector([1, 0, 0])))],
    });

    await client.execute({
      sql: `INSERT INTO memories (id, content, embedding) VALUES (?, ?, vector(?))`,
      args: ["opposite", "Opposite direction", JSON.stringify(Array.from(createNormalizedVector([-1, 0, 0])))],
    });
    ...
    const sameDistance = result.rows.find((r) => r.id === "same")?.distance as number;
    expect(sameDistance).toBeLessThan(0.1);

    // Opposite direction should have distance ~2
    const oppositeDistance = result.rows.find((r) => r.id === "opposite")?.distance as number;
...
describe("Swarm-Mail Database Schema Tests", () => {
  ...
  test("events table supports concurrent inserts", async () => {
    ...
    const count = await db.query<{ cnt: number }>("SELECT COUNT(id) as cnt FROM events");
