...
describe("MemoryAdapter - Semantic Search", () => {
	...
	beforeEach(async () => {
		...
		const mockFetch = mock((url: string, options?: RequestInit) => {
			if (url.includes("/api/embeddings")) {
				...
				const seed = prompt.includes("TypeScript")
					? 1
					: prompt.includes("JavaScript")
						? 1.1
						: prompt.includes("cooking") || prompt.includes("pasta")
							? 50
							: 1.5;
		...
		await adapter.store("Cooking pasta requires boiling water", {
			collection: "food",
		});
	});
	...
	describe("MemoryAdapter - Smart Upsert (Mem0 Pattern)", () => {
		...
		test.skipIf(!hasWorkingLLM)(
			"upsert() with refined information returns UPDATE (requires working LLM)",
			async () => {
		...
		test.skipIf(!hasWorkingLLM)(
			"upsert() with contradicting information returns DELETE (requires working LLM)",
			async () => {
    ...
    `);
		...
		test("findByEntity() searches through entity graph", async () => {
			// Create memories
			const mem1 = await adapter.store(
				"TypeScript is Joel's preferred language",
			);
			...
			expect(results.some((r) => r.memory.content.includes("preferred"))).toBe(
				true,
			);
			expect(results.some((r) => r.memory.content.includes("teaches"))).toBe(
				true,
			);
		});
	...
	describe("MemoryAdapter - Enhanced Store with Auto Features", () => {
		...
		test.skipIf(!hasWorkingLLM)(
			"store() with autoTag extracts tags from content (requires working LLM)",
			async () => {
    ...
    `);
		...
		test("store() with extractEntities integration test", async () => {
			...
			const { storeEntities, storeRelationships, linkMemoryToEntities } =
