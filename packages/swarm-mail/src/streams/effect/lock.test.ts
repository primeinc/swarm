...
describe("DurableLock", () => {
		...
		it("should fail when lock is held by another holder", async () => {
			const program = Effect.gen(function* (_) {
				...
				const result2 = yield* _(
					acquireLock(
						"test-resource",
						config({ maxRetries: 2, baseDelayMs: 10 }),
					).pipe(Effect.either),
				);
		...
		it("should allow same holder to re-acquire lock", async () => {
			const program = Effect.gen(function* (_) {
				...
				const lock1 = yield* _(
					acquireLock(
						"test-resource",
						config({ holderId: holder, ttlSeconds: 10 }),
					),
				);
	...
	describe("Lock expiry (TTL)", () => {
		it("should allow acquisition after lock expires", async () => {
			const program = Effect.gen(function* (_) {
				...
				const lock1 = yield* _(
					acquireLock("test-resource", config({ ttlSeconds: 1 })),
				);
	...
	describe("withLock helper", () => {
		...
		it("should pass through effect result", async () => {
			const program = Effect.gen(function* (_) {
				const result = yield* _(
					withLock("test-resource", Effect.succeed(42), config()),
				);
	...
	describe("Configuration", () => {
		it("should respect custom TTL", async () => {
			const program = Effect.gen(function* (_) {
				const ttlSeconds = 5;
				const lock = yield* _(
					acquireLock("test-resource", config({ ttlSeconds })),
				);
		...
		it("should respect custom holder ID", async () => {
			const program = Effect.gen(function* (_) {
				...
				const lock = yield* _(
					acquireLock("test-resource", config({ holderId })),
				);
