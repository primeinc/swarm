...
async function main() {
	...
	console.log(
		`   ${similarity > 0.7 ? "✅ High similarity - semantic search would match!" : "⚠️ Low similarity"}\n`,
	);
	...
	const dissimilarity = embedding.reduce(
		(sum, val, i) => sum + val * embedding3[i],
		0,
	);
	console.log(
		`   📐 Cosine similarity with unrelated text: ${dissimilarity.toFixed(4)}`,
	);
	console.log(
		`   ${dissimilarity < 0.3 ? "✅ Low similarity - correctly different!" : "⚠️ Unexpectedly similar"}\n`,
	);
