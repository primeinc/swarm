...
Windows paths have several problematic formats that cause "file not found" errors:

1. **Missing slash after drive letter**: `C:Users` instead of `C:/Users`
2. **Mixed path separators**: `C:\Users/will\dev` mixing `\` and `/`
3. **Multiple consecutive slashes**: `C://Users///will` from buggy string manipulation
4. **UNC path corruption**: `\\server\share` incorrectly treated as regular paths

**Why It Happens:**
- Windows allows `C:` without trailing slash, but Node.js expects `C:/`
- Direct string concatenation creates malformed paths: `'C:' + 'Users'` → `C:Users`
- Mixing `path.join()` with string concatenation produces inconsistent separators
- SQLite stores paths as-is, causing comparison failures

#### Symptoms

**Error Messages:**
```
ENOENT: no such file or directory, open 'C:Userswillfile.txt'
No matches found (grep pattern: C:\Users\will\src\**\*.ts)
File reservation failed: path not found
```

**Grep Failures:**
```typescript
// Pattern fails because backslashes not normalized
grep({ pattern: 'src\\**\\*.ts' }); // Treats \\ as escape sequences
```

**Path Comparison Issues:**
```typescript
// These look identical but fail strict equality
'C:\\Users\\will\\dev\\file.ts'  !== 'C:/Users/will/dev/file.ts'
// Database query misses reservation
```

#### Affected Operations

- `fs.readFile()`, `fs.writeFile()`, `fs.stat()` - All filesystem operations
- `grep()`, `ast_grep()` - Pattern matching fails with backslashes
- `swarmmail_reserve()` - Path comparison for reservations
- SQLite path storage and queries
...
Created comprehensive path normalization utility at `src/utils/normalize-path.ts`:
...
// => ['C:/Users/will/src/file1.ts', 'D:/project/src/file2.ts']
```
...
**Best Practices:**
1. **Always normalize user input** before file operations
2. **Normalize before SQLite storage** to ensure path comparison works
3. **Normalize grep patterns** to handle Windows backslashes
4. **Use path.join() instead of string concatenation**
5. **Test with both backslash and forward slash inputs**
...
// ✅ GOOD - Use path.join + normalize
import * as path from 'path';
import { normalizePath } from './utils/normalize-path';

const good = normalizePath(path.join('C:/', 'Users', 'will'));
```
...
- **Tests:** `src/utils/normalize-path.test.ts`
- **Documentation:** `docs/windows/10-grep-fixes.md`
- **Hivemind:** `mem-df6be947bc3e363b`, `mem-c02e06b9f5f39978`
- **Session:** `ses_4446fc875ffeNL42aPfoysDLem` (2026-01-14)
...
The `swarm_review_feedback` tool expects the `issues` parameter as a **STRING containing JSON**, not a JSON object directly. On Windows, shell escaping often corrupts the JSON string before it reaches the tool.

**Technical Details:**
- Tool definition: `issues: string().optional().describe("JSON array of ReviewIssue objects")`
- Parsing logic: `JSON.parse(args.issues)` expects a string
- Windows PowerShell/CMD have different escaping rules than Unix shells
- Double-nested quoting causes corruption: `"\"[]\""` instead of `"[]"`
...
**Common Failures:**
```typescript
// Test 1: Double-escaped quotes
{ issues: "\"[]\"" }  // PowerShell added extra quotes
→ JSON.parse fails

// Test 2: Truncated string
{ issues: "[" }  // Shell truncated the input
→ JSON.parse fails
...
- `swarm_review_feedback()` with `status: "needs_changes"`
- Coordinator sending review feedback to workers
- Adversarial review loops
- Multi-attempt review workflows

#### Verified Workarounds

**Solution 1: Omit Parameter for Approval (Recommended)**

```typescript
// ✅ BEST - Omit issues parameter entirely for approval
...
**Why This Works:** JavaScript's `if (args.issues)` treats undefined as falsy, skipping the parse attempt entirely.

**Solution 2: Properly Stringify JSON for Rejection**

```typescript
// ✅ GOOD - Use JSON.stringify for needs_changes
await swarm_review_feedback({
  project_key: "C:\\Users\\will\\dev\\swarm-tools",
  task_id: "epic-123.task-456",
  worker_id: "worker-1",
...
**Solution 3: PowerShell CLI Usage**
...
$issues = @'
[{"file":"test.ts","severity":"error","message":"test"}]
'@
...
**Tool Users:**
1. For approval, omit `issues` parameter entirely
2. For rejection, always use `JSON.stringify()`
3. Test JSON string in Node.js REPL before using:
   ```javascript
   JSON.parse('[{"file":"test.ts","severity":"error","message":"test"}]')
   ```

**Tool Developers (Future Enhancement):**
```typescript
// Better error messages
  ...
  return JSON.stringify({
    ...
    received: args.issues  // Show what was actually received
  }, null, 2);
}
...
- **Investigation:** `docs/windows/11-review-feedback-investigation.md`
- **Hivemind:** `mem-336b3e667a892b49`
- **Session:** `ses_44453859dffeqS3DxrF8hRpKNB` (2026-01-14)
- **Tool Source:** `~/.bun/install/global/node_modules/opencode-swarm-plugin/dist/bin/swarm.js`
