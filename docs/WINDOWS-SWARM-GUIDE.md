...
**Native Windows (PowerShell/CMD) vs. Unix Shells:**

While core swarm logic (reservations, progress tracking) is being hardened for native Windows, several recovery and installation features currently **require a Unix-style shell** (Git Bash or WSL):

1.  **Process Management**: Tools that identify and kill hung coordination servers (`lsof`, `kill`) are Unix-only.
2.  **Installation**: CLI setup scripts often spawn `bash` for dependency management.
3.  **Self-Healing**: Automated recovery flows in `agent-mail.ts` currently depend on POSIX binaries.

**Recommendation:** For the most stable experience on Windows, execute all swarm operations from **Git Bash** or within a **WSL** instance. Native PowerShell support is currently in a "Transitional" state.
