...
Use cell IDs as thread IDs for all coordination messages.
...
// Also update cell status
cells_update({ id: "bd-123.2", status: "blocked" });
```
...
```
❌ Agent spawns → works silently for 45 minutes → closes cell
...
const result = await Task({
  ...
  prompt: "Generate cellTree for: " + task
});
...
cells_query({ status: "in_progress" })
...
cells_close({ id: "bd-123", reason: "Done: implemented auth" })
